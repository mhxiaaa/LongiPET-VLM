...
from dataset.assistFun import get_patchList_Test, reverseSeg, crop_image_zeroOut3D, uncrop_image_zeroOut3D
from model.arch_vision import Vision, VisionConfig
import SimpleITK as sitk
from pathlib import Path
import numpy as np
# /home2/mx79/Anaconda3/bin/python /home2/mx79/code_PET-VLM_6/eval/eval0_vision_seg.py
...
def parse_args(args=None):
    ...
    parser.add_argument('--output_dir', type=str, default="/data22/user/mx79/results_code-PET-VLM_6/eval0_vision_seg/")
...
def main():
    ...
    with open("/data22/user/mx79/0.___allTaskSort_file_path/SEG.json", "r", encoding="utf-8") as f: 
        data = json.load(f)
    path_SEGtask_train, path_SEGtask_val, args.seg_data_test_path = data["train"], data["val"], data["test"]
    ...
    for file in Path('/data17/user/mx79/03.Downstream_mCT_FDG_from_dynamicData/').rglob('*.nii.gz'):
        if 'Seg.nii.gz' not in str(file) and '/CT_' not in str(file) and 'HC.nii.gz' not in str(file):
            low_path = str(file)
            seg_path = low_path.replace(low_path.split('/')[-1], 'tumorSeg.nii.gz')
            dose = int(low_path.split('/')[-1].replace('.nii.gz', '').replace('_', ''))
            args.seg_data_test_path.append({"image": low_path, "seg": seg_path, "dose_tag": dose})

    print('num of pairs for testing seg task:', len(args.seg_data_test_path))
    ...
    _test_indices = list(range(8, 10)) + list(range(418, 420)) + list(range(518, 520))
    # for tt in [args.seg_data_test_path[i] for i in _test_indices]:
    for tt in args.seg_data_test_path:
        image_path, gt_path, dose = tt["image"], tt["seg"], tt["dose_tag"]
        ...
        if not os.path.exists(savePath):
            ...
            for ppIdx in range(0, len(patchIdxAll), args.patchBatchSize):
                ...
                with torch.inference_mode():
                    predS = model.forward_seg(images=lowS, images_gt=None,
            ...
            predIm_result = reverseSeg(img_size=image.shape, predictions=patchAll_result, patchIdx=patchIdxAll)
            predIm_result = predIm_result.argmax(0)
            ...
            predSave = sitk.GetImageFromArray(predFinal.astype(np.uint8))
