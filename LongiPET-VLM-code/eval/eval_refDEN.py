...
if local_server:
    ...
    save_dir = '/data22/user/mx79/'
    dst_dir = '/data17/user/mx79/'
else:
    os.environ["CUDA_HOME"] = os.environ.get("EBROOTCUDA", "/apps/software/2024a/software/CUDA/12.6.0")
    os.environ["HF_HUB_CACHE"] = "/home/mx79/project_pi_cl598/mx79/huggingface/hub"
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    save_dir = '/home/mx79/project_pi_cl598/mx79/'
    dst_dir = '/home/mx79/project_pi_cl598/mx79/0dataset/'
import json
import random
import numpy as np
import argparse
...
from dataset.assistFun import get_patchList_Test, reverseImg, crop_image_zeroOut3D, uncrop_image_zeroOut3D, crop_like_with_meta
from model.llm_qwen import VLMQwenForCausalLM, VLMQwenConfig  # registers vlm_qwen with AutoModel
from tqdm import tqdm
import SimpleITK as sitk
from pathlib import Path

src_dir = '/data17/user/mx79/'

# python3 /home/mx79/project_pi_cl598/mx79/LongiPET-VLM/eval/eval_refDEN.py
...
def parse_args(args=None):
    ...
    parser.add_argument("--model_name_or_path", type=str, default='/home/mx79/project_pi_cl598/mx79/LongiPET-VLM-model_results/model')
    parser.add_argument("--max_length", type=int, default=4096)
    parser.add_argument("--proj_out_num", type=int, default=512)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--output_dir", type=str, default='/home/mx79/project_pi_cl598/mx79/LongiPET-VLM-model_results/eval_refDEN/')
...
def main():
    ...
    with open("/home/mx79/project_pi_cl598/mx79/LongiPET-VLM-dataPath/DEN.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    _, _, args.vae_data_test_path = data["train"], data["val"], data["test"]
    print('num of pairs for testing den (from DEN.json):', len(args.vae_data_test_path))

    for file in Path(dst_dir + "03.Downstream_mCT_FDG_from_dynamicData/").rglob('*.nii.gz'):
        if 'Seg.nii.gz' not in str(file) and '/CT_' not in str(file) and 'HC.nii.gz' not in str(file):
            ...
            args.vae_data_test_path.append({"low_dose": low_path, "high_dose": high_path, "dose_tag": dose, "previous_high_doses": []})
    print('num of pairs for testing den (total):', len(args.vae_data_test_path))
    ...
    for item in args.vae_data_test_path:
        image_path = item["low_dose"].replace(src_dir, dst_dir)
        dose = item["dose_tag"]
        ...
        question = "<im_patch>" * args.proj_out_num + ' Task: DEN. ' + priorInfo
        ...
        for ppIdx in range(0, len(patchIdxAll), args.patchBatchSize):
            ...
            with torch.inference_mode():
                predS = model.generate(images=lowS, images_pre=preS, images_pre_mask=preS_mask,
                                       ...
                                       task_type='DEN',)
                predS[predS <= 0.0] = 0.0
            predS = predS[:, 0, :, :, :].detach().to(torch.float32).cpu().numpy()
            for bbIdx in range(BatchSizeCurrent):
        ...
        predFinal = uncrop_image_zeroOut3D(predIm_result, meta_pad)
