...
def parse_args(args=None):
    ...
    parser.add_argument("--output_dir", type=str, default="/home/mx79/project_pi_cl598/mx79/LongiPET-VLM-model_results/eval_refSEG/")
...
def main():
    ...
    with open("/home/mx79/project_pi_cl598/mx79/LongiPET-VLM-dataPath/SEG.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    for file in Path(dst_dir + "03.Downstream_mCT_FDG_from_dynamicData/").rglob('*.nii.gz'):
        if 'Seg.nii.gz' not in str(file) and '/CT_' not in str(file) and 'HC.nii.gz' not in str(file):
            ...
            args.seg_data_test_path.append({"image": low_path, "seg": seg_path, "dose_tag": dose, "previous_high_doses": []})
    print('num of pairs for testing seg (total):', len(args.seg_data_test_path))
