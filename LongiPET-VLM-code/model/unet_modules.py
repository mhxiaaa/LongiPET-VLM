...
class GroupNorm32(nn.GroupNorm):
    def forward(self, x):
        # GroupNorm in float32 for stability; params cast to float32 to match
        orig_dtype = x.dtype
        weight = self.weight.float() if self.weight is not None else None
        bias = self.bias.float() if self.bias is not None else None
        return th.nn.functional.group_norm(x.float(), self.num_groups, weight, bias, self.eps).to(orig_dtype)
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1).to(self.qkv.weight.dtype)
